      ...
      <section className="pt-36 pb-16 text-center relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <div className="w-14 h-14 rounded-2xl bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center text-cyan-600 dark:text-cyan-400 mx-auto mb-6">
      ...
      <section className="py-12 border-t border-slate-300 dark:border-white/5">
        <div className="container mx-auto px-6 max-w-6xl">
          <div className="grid lg:grid-cols-5 gap-12">
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              {contacts.map((c, i) => (
                <motion.div
                  ...
                  className="flex items-start gap-4 p-5 bg-white shadow-sm dark:bg-slate-900/60 border border-slate-300 dark:border-white/5 rounded-2xl hover:border-slate-300 dark:border-white/10 transition-colors"
                >
                  <div className="w-10 h-10 rounded-xl bg-cyan-500/10 flex items-center justify-center text-cyan-600 dark:text-cyan-400 flex-shrink-0">
                    <c.icon className="w-5 h-5" />
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-900 dark:text-slate-500 font-bold uppercase tracking-wider block mb-0.5">{c.label}</span>
                    <p className="text-sm font-semibold text-slate-950 dark:text-white font-medium">{c.value}</p>
                    <p className="text-xs text-slate-900 dark:text-slate-500 mt-0.5">{c.sub}</p>
                  </div>
                </motion.div>
              ))}

              {/* Map placeholder */}
              <div className="mt-6 rounded-2xl overflow-hidden border border-slate-300 dark:border-white/5 aspect-video relative bg-white dark:bg-slate-900">
                ...
                <div className="absolute inset-0 flex items-center justify-center flex-col gap-2 text-slate-900 dark:text-slate-500">
            ...
            >
              <div className="bg-white shadow-sm dark:bg-slate-900/60 border border-slate-300 dark:border-white/10 rounded-3xl p-8">
                {submitted ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-slate-600 dark:text-slate-400 text-sm">We'll get back to you at <span className="text-cyan-600 dark:text-cyan-400">{form.email}</span> within 24 hours.</p>
                    <button
                      onClick={() => { setSubmitted(false); setForm({ name: "", email: "", subject: "", message: "" }); }}
                      className="mt-6 px-5 py-2.5 text-sm text-slate-700 dark:text-slate-300 border border-white/15 rounded-xl hover:bg-white/5 transition-colors"
                    >
                      Send Another
                    </button>
                  </div>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-5" noValidate>
                    ...
                    <div className="grid sm:grid-cols-2 gap-5">
                      <div>
                        ...
                        <input
                          ...
                          className={`w-full bg-white shadow-sm dark:bg-slate-950/60 border rounded-xl py-3 px-4 text-sm text-slate-950 dark:text-white font-medium focus:outline-none focus:ring-1 transition-all ${errors.name ? 'border-rose-500/50 focus:ring-rose-500/30' : 'border-slate-300 dark:border-white/10 focus:border-cyan-500/50 focus:ring-cyan-500/30'}`}
                        />
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className={`w-full bg-white shadow-sm dark:bg-slate-950/60 border rounded-xl py-3 px-4 text-sm text-slate-950 dark:text-white font-medium focus:outline-none focus:ring-1 transition-all ${errors.email ? 'border-rose-500/50 focus:ring-rose-500/30' : 'border-slate-300 dark:border-white/10 focus:border-cyan-500/50 focus:ring-cyan-500/30'}`}
                        />
                    ...
                    <div>
                      <label className="text-[10px] text-slate-900 dark:text-slate-500 font-bold uppercase tracking-wider block mb-2">Subject</label>
                      <input
                        value={form.subject}
                        onChange={e => setForm(f => ({ ...f, subject: e.target.value }))}
                        placeholder="Enterprise partnership inquiry"
                        className={`w-full bg-white shadow-sm dark:bg-slate-950/60 border rounded-xl py-3 px-4 text-sm text-slate-950 dark:text-white font-medium focus:outline-none focus:ring-1 transition-all ${errors.subject ? 'border-rose-500/50 focus:ring-rose-500/30' : 'border-slate-300 dark:border-white/10 focus:border-cyan-500/50 focus:ring-cyan-500/30'}`}
                      />
                      {errors.subject && <p className="text-rose-400 text-[10px] mt-1 font-semibold">{errors.subject}</p>}
                    </div>

                    <div>
                      <label className="text-[10px] text-slate-900 dark:text-slate-500 font-bold uppercase tracking-wider block mb-2">Message</label>
                      <textarea
                        rows={6}
                        value={form.message}
                        onChange={e => setForm(f => ({ ...f, message: e.target.value }))}
                        placeholder="Tell us about your parking lot, number of slots, or any specific integration needs..."
                        className={`w-full bg-white shadow-sm dark:bg-slate-950/60 border rounded-xl py-3 px-4 text-sm text-slate-950 dark:text-white font-medium focus:outline-none focus:ring-1 transition-all resize-none ${errors.message ? 'border-rose-500/50 focus:ring-rose-500/30' : 'border-slate-300 dark:border-white/10 focus:border-cyan-500/50 focus:ring-cyan-500/30'}`}
                      />
