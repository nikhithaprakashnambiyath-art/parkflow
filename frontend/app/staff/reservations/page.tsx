...
const statusColors: Record<string, string> = {
  ...
  CANCELLED: "bg-slate-700/40 text-slate-600 dark:text-slate-400 border-slate-300 dark:border-white/5",
};
      ...
      <main className="flex-1 p-8 lg:p-10 overflow-y-auto relative z-10">
        <header className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-xs text-slate-600 dark:text-slate-400 uppercase tracking-widest font-bold mt-1">{filtered.length} reservations shown</p>
          </div>
          <div className="flex gap-2">
            {(["ALL", "ACTIVE", "PENDING", "COMPLETED"] as const).map(s => (
              <button key={s} onClick={() => setStatusFilter(s)}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all ${statusFilter === s ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/30" : "border-slate-300 dark:border-white/5 text-slate-600 dark:text-slate-400 hover:border-white/15"}`}>
                {s}
              </button>
            ))}
          </div>
        ...
        <div className="space-y-3">
          {filtered.map((b, i) => (
            <motion.div key={b.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="space-y-1.5">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-[10px] font-mono text-slate-900 dark:text-slate-500"># {b.id.slice(0, 8)}</span>
                  </div>
                  <p className="text-sm font-black text-slate-950 dark:text-white font-medium">{b.user?.name}</p>
                  <p className="text-xs text-slate-900 dark:text-slate-500">{b.user?.email}</p>
                  <p className="text-xs text-slate-600 dark:text-slate-400 flex items-center gap-1">
                    <MapPin className="w-3 h-3 text-cyan-600 dark:text-cyan-400" /> {b.slot?.lot?.name} · Slot {b.slot?.name}
                  </p>
                  <p className="text-[10px] text-slate-900 dark:text-slate-500 flex items-center gap-1">
              ...
              <div className="flex sm:flex-col justify-between items-center sm:items-end gap-3 shrink-0 border-t sm:border-0 border-slate-300 dark:border-white/5 pt-3 sm:pt-0">
                <span className="text-lg font-black text-cyan-600 dark:text-cyan-400">{formatCurrency(b.amount)}</span>
                <div className="flex gap-2">
                  {b.status === "PENDING" && (
                  ...
                  <button onClick={() => router.push(`/staff/scanner`)}
                    className="text-xs font-bold text-slate-600 dark:text-slate-400 border border-slate-300 dark:border-white/10 hover:border-white/20 px-3 py-2 rounded-xl transition-all flex items-center gap-1">
          ...
          {filtered.length === 0 && (
            <div className="bg-white shadow-sm dark:bg-slate-900/40 border border-slate-300 dark:border-white/5 rounded-2xl p-12 text-center text-slate-900 dark:text-slate-500 italic">No reservations match this filter.</div>
          )}
        </div>
      </main>
    </div>
