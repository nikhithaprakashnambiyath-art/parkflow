...
export default function LoginPage() {
  ...
  const handleGoogleMock = async () => {
    ...
    try {
      let res;
      if (role === 'ADMIN') {
        res = await login('admin@example.com', 'admin123');
      } else {
        res = await login('alex@example.com', 'password123');
      }
      ...
      setErrorMsg(err.message || 'Quick-test login failed. Use the email/password form instead.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950 text-slate-900 dark:text-slate-50 flex items-center justify-center p-6 relative selection:bg-cyan-500/30 overflow-hidden">
      ...
      <div className="w-full max-w-md bg-white shadow-sm dark:bg-slate-900/60 border border-slate-300 dark:border-white/10 backdrop-blur-xl rounded-3xl p-8 shadow-2xl relative z-10 overflow-hidden">
        ...
        <div className="grid grid-cols-2 p-1 bg-slate-100 dark:bg-slate-950 border border-slate-300 dark:border-white/5 rounded-2xl mb-6 relative">
          ...
          {/* Active Tab Sliding Overlay */}
          <motion.div
            layoutId="authTabSlider"
            className="absolute top-1 bottom-1 left-1 w-[calc(50%-4px)] bg-cyan-400 rounded-xl"
        ...
        <AnimatePresence>
          {errorMsg && (
            <motion.div 
              ...
              className="p-3 mb-4 rounded-xl border border-rose-500/20 bg-rose-500/10 text-rose-400 text-xs font-semibold"
            >
              {errorMsg}
            </motion.div>
          )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-bold py-4 rounded-xl shadow-[0_0_20px_rgba(0,217,255,0.2)] mt-6 text-sm flex gap-2 justify-center items-center disabled:opacity-50 disabled:pointer-events-none transition-colors"
          >
            {isLoading ? (
              <motion.div
                animate={{ rotate: 360 }}
        ...
        >
          ...
          <span>Use Quick-Test Account</span>
        </Button>
