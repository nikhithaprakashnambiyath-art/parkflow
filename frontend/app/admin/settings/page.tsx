...
export default function AdminSettingsPage() {
  ...
  const Toggle = ({ value, onChange, label }: { value: boolean; onChange: (v: boolean) => void; label: string }) => (
    <div className="flex justify-between items-center py-3 border-b border-slate-300 dark:border-white/5 last:border-0">
    ...
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950 text-slate-900 dark:text-slate-50 flex selection:bg-cyan-500/30">
      ...
      <aside className="w-64 border-r border-slate-300 dark:border-white/10 bg-white shadow-sm dark:bg-slate-950/60 backdrop-blur-xl flex flex-col p-6 sticky top-0 h-screen z-20 shrink-0">
        <div className="flex items-center gap-2.5 mb-10 cursor-pointer" onClick={() => router.push("/")}>
          <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-cyan-400 to-blue-600 flex items-center justify-center font-bold text-slate-950 dark:text-white font-medium">
      ...
      <main className="flex-1 p-8 lg:p-12 overflow-y-auto">
        <header className="flex justify-between items-center mb-10">
          <div>
            ...
            <p className="text-xs text-slate-600 dark:text-slate-400 uppercase tracking-widest font-bold mt-1">Platform configuration & operational controls</p>
          </div>
          <button
            onClick={handleSave}
            disabled={saving}
        ...
        <div className="grid lg:grid-cols-2 gap-6 max-w-5xl">
          ...
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
            className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-white/5 rounded-3xl p-6">
            <h2 className="text-sm font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 flex items-center gap-2 mb-5">
              <Globe className="w-4 h-4 text-cyan-600 dark:text-cyan-400" /> General
            </h2>
            <div className="space-y-4">
              {[
                ...
                <div key={f.label}>
                  <label className="text-[10px] text-slate-900 dark:text-slate-500 font-bold uppercase tracking-wider block mb-2">{f.label}</label>
                  <input value={f.value} onChange={e => f.setter(e.target.value)}
                    className="w-full bg-slate-100 dark:bg-slate-950 border border-slate-300 dark:border-white/10 focus:border-cyan-500/50 rounded-xl py-2.5 px-4 text-sm text-slate-950 dark:text-white font-medium focus:outline-none transition-all" />
                </div>
              ))}
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
            className={`rounded-3xl p-6 border ${maintenanceMode ? "bg-rose-500/5 border-rose-500/20" : "bg-white dark:bg-slate-900 border-slate-300 dark:border-white/5"}`}>
            <h2 className="text-sm font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 flex items-center gap-2 mb-5">
            ...
            <div className="mt-5 pt-5 border-t border-slate-300 dark:border-white/5">
              <p className="text-[10px] text-slate-900 dark:text-slate-500 font-bold uppercase tracking-wider mb-3">Danger Zone</p>
              <button
