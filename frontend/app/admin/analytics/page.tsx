      ...
      <main className="flex-1 p-8 lg:p-10 overflow-y-auto relative z-10">
        <header className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-xs text-slate-400 uppercase tracking-widest font-bold mt-1">Real-time performance insights</p>
          </div>
          <div className="flex items-center gap-2">
            {(["7d", "30d", "90d"] as const).map(p => (
              <button key={p} onClick={() => setPeriod(p)}
                className={`px-4 py-2 rounded-xl text-xs font-bold border transition-all ${period === p ? "bg-cyan-500/10 text-cyan-400 border-cyan-500/30" : "border-white/5 text-slate-400 hover:border-white/15"}`}>
        ...
        <div className="grid grid-cols-4 gap-5 mb-8">
          {[
            ...
            <motion.div key={i} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }}
              ...
              <span className="text-[10px] text-slate-500 uppercase tracking-wider font-bold block mb-1">{kpi.label}</span>
            ...
            <div className="flex justify-between items-center mb-6">
              <div>
                <h3 className="text-sm font-bold uppercase tracking-wider text-slate-300 flex items-center gap-2">
                  <TrendingUp className="w-4 h-4 text-cyan-400" /> Revenue Trend
                </h3>
                <p className="text-xs text-slate-500 mt-0.5">Daily revenue vs bookings</p>
              </div>
            </div>
            <div className="h-56">
